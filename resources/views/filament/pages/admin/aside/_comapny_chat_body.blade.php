        ...
        <div
            ...
            <div
                ...
                <div class="relative flex items-center space-x-4 ml-5">
                    @php $receiver = $current_room->members()->whereAgent()->first()->memberable; @endphp
                    <div class="relative">
                        <img
                            src="{{ $receiver->company->logo ?? 'https://ui-avatars.com/api/?name=' . urlencode(__('attributes.chat.deleted_company')) . '&color=FFFFFF&background=111827' }}"
                            alt="{{ $receiver->company->name ?? __('attributes.chat.deleted_company') }}"
                            class="w-10 lg:w-11 h-10 lg:h-11 rounded-full">
                    </div>
                    <div class="flex flex-col leading-tight">
                        <div class="text-lg mt-1 flex items-center">
                            <span
                                class="text-gray-700 mr-3">{{ $receiver->company->name ?? __('attributes.chat.deleted_company') }}</span>
                        </div>
                        <div class="text-sm text-gray-600">
                            <span class="mr-3">{{ $receiver->company->email ?? __('attributes.chat.deleted_company') }}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    @else
                        <div class="chat-message">
                            <div class="flex items-start">
                                <img src="{{ $message->sender->memberable?->company->logo ?? 'https://ui-avatars.com/api/?name=' . urlencode(__('attributes.chat.deleted_company')) . '&color=FFFFFF&background=111827' }}" alt="My profile"
