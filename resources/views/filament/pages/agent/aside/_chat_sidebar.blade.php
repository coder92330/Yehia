<div class="h-full w-full lg:col-span-3 col-span-2">
    ...
    @foreach($rooms as $room)
        @php
            $reciver  = $room->members()->when(isset($room->chat_type) && $room->chat_type === 'company_chat' ,fn($q) => $q->whereUser(), fn($q) => $q->whereTourguide())->first();
        ...
        @if($sideRoom)
            @if(!is_null($room->event))
                <div class="flex justify-between py-4 border-gray-200 cursor-pointer
                     ...
                     wire:click="selectRoom({{ $reciver->memberable_id }}, {{ $room->event->id }})">
                    ...
                    <div class="lg:flex flex-col items-center space-y-4 mr-5">
                        @if($reciver->messages()->whereRoomId($room->id)->exists())
                            <span
                                class="text-xs text-gray-500">{{ $reciver->messages()->lastMessage($room->id)->created_at->diffForHumans() }}</span>
                        @endif
                        <span
