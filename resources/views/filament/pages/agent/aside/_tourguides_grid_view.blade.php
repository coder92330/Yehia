        ...
        <div class="col-span-1">
            <div
                ...
                <div
                    ...
                    <div class="relative p-4 sm:p-6 lg:p-8 h-full">
                        <div @class([
                            ...
                            'flex-col' => $tourguide->hasSetting('my_profile_added_to_favorites', true)->exists(),
                            'flex-col-reverse' => !$tourguide->hasSetting('my_profile_added_to_favorites', true)->exists(),
                        ])>
                            @if($tourguide->hasSetting('my_profile_added_to_favorites', true)->exists())
                                <div
                                    class="flex @if($tourguide->favourites()->where('favouriter_type', App\Models\User::class)->exists()) items-start justify-between @else justify-end @endif">
                                <span
                                    class="px-4 py-0.5 rounded-full bg-primary-600 text-xs text-white
                                    @if(!$tourguide->favourites()->where('favouriter_type', App\Models\User::class)->exists()) hidden @endif">Recommended</span>
                                    <x-filament::button class="w-5 h-5"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                                             fill="currentColor"
                                             class="w-5 h-5 inline @if(auth('agent')->user()->favourites()->where(['favouritable_id' => $tourguide->id,'favouritable_type' => \App\Models\Tourguide::class])->exists()) text-danger-500 @else text-white @endif">
                                            <path
                            ...
                            <a href="{{ route($route, $tourguide->id) }}"
                               class="flex flex-col items-center">
                                <span
                                    class="text-md text-white">{{ $tourguide->full_name }}</span>
                                <span
                                    class="text-xs text-gray-400 ">{{ $tourguide->email }}</span>
                                <span
                ...
                <p class="px-6 text-center py-4 font-normal text-sm text-gray-700 dark:text-gray-400">
                    &ldquo; {{ $tourguide-> bio ?? 'Every hundred feet the world changes' }} &ldquo;</p>
                        ...
                        <span class="inline-flex items-center text-xs">
                            @for($i = 0; $i < 5; $i++)
                                <svg aria-hidden="true"
                                     ...
                                     fill="currentColor"
                                     viewBox="0 0 20 20"
                                     xmlns="http://www.w3.org/2000/svg"><title>First star</title><path
