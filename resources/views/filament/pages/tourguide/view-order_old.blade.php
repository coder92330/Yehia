    ...
    <div class="grid grid-cols-4 gap-4 mt-4">
        <div class="col-span-3">
            <div class="max-h-96 overflow-hidden rounded-2xl mb-4">
                <img src="{{ $record->event->cover }}" alt="{{ $record->event->name }}" class="rounded-2xl">
            </div>
                    ...
                    @foreach($record->event->days as $day)
                        <div class="my-6">
                            <h1 class="text-md font-semibold border-b border-primary-500 p-2 w-32 text-center">
                                {{ $day->start_at->format('M d') }}</h1>
                            <div class="grid grid-cols-4 gap-4">
                                @foreach($day->sessions as $session)
                                    <div class="col-span-1 p-4">
                                        <p class="text-sm text-gray-500 dark:text-gray-400">
                                            {{ $session->start_at->format('h:i A') }}
                                            - {{ $session->end_at->format('h:i A') }}
                                        </p>
                                        <p class="text-sm text-gray-500 dark:text-gray-400">
                                            {{ $session->location }}
                                        </p>
        ...
        <div class="col-span-1">
            <div class="flex flex-col gap-2 items-center">
                <div class="flex items-center border border-gray-200 rounded-lg p-6 bg-white w-full">
                    <img
                        src="https://ui-avatars.com/api/?name={{$record->event->agent->company->name}}&color=7F9CF5&background=EBF4FF"
                        class="w-14 h-14 rounded-full" alt="">
                    <div class="ml-4">
                        <small class="text-gray-500">Presented By</small>
                        <span class="text-sm font-semibold block">{{ $record->event->agent->company->name }}</span>
                    </div>
                </div>
                <div
                    class="@if(!auth('tourguide')->check() ||
                        ($record->status === 'approved' && $record->agent_status === 'approved')) hidden @endif
                    ...
                    @endif
                    @if($record->status !== 'rejected' && $record->agent_status !== 'approved')
                        <x-filament::button
