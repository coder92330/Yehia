...
class Settings extends Page
{
    ...
    {
        return [
            route('tour-guide.pages.dashboard') => 'Dashboard',
    ...
    {
        $this->settings = auth('tourguide')->user()->settings()->get();
    ...
    public function submit()
    {
        try {
            ...
            $this->settings->each(function ($setting) {
                auth('tourguide')->user()->settings()->syncWithoutDetaching([$setting->id => ['value' => $this->form->getState()[$setting->label]]]);
            });
            Notification::make()->success()->title(__('notifications.updated_successfully', ['field' => __('attributes.settings')]))->send();
            DB::commit();
        } catch (\Exception $e) {
            DB::rollBack();
            Log::channel('tourguide')->error("Error in Setting@submit: {$e->getMessage()} in File: {$e->getFile()} on Line: {$e->getLine()}");
