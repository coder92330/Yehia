...
use App\Http\Requests\Api\V1\Agent\Setting\UpdateSettingRequest;
use App\Http\Resources\Api\V1\Agent\PageResource;
...
class SettingsController extends Controller
{
    ...
    {
        return ($settings = auth('agent_api')->user()->settings)
            ? SettingsResource::collection($settings)
            : SettingsResource::collection($settings)->additional(['message' => __('messages.not_found', ['attribute' => __('attributes.settings')])]);
    }
    ...
    public function update(UpdateSettingRequest $request): SuccessResource|ErrorResource
    {
        try {
            ...
            DB::rollBack();
            Log::channel('agent')->error("Error in SettingsController@update: {$e->getMessage()} at Line: {$e->getLine()} in File: {$e->getFile()}");
