...
use App\Http\Requests\Api\V1\Tourguide\DeviceKey\DeviceKeyRequest;
use App\Http\Resources\Api\V1\ErrorResource;
use App\Http\Resources\Api\V1\SuccessResource;
use App\Http\Resources\Api\V1\Tourguide\NewsNotificationResource;
use App\Http\Resources\Api\V1\Tourguide\NotificationResource;
...
class NotificationController extends Controller
{
    ...
    {
        $notifications = auth('tourguide_api')->user()->notifications()->paginate(config('app.pagination'));
        return count($notifications) > 0
            ? NotificationResource::collection($notifications)->additional(['un_readed_notifications' => auth('tourguide_api')->user()->unreadNotifications()->count()])
            : NotificationResource::collection($notifications)->additional(['message' => __('messages.no_notifications')]);
    }

    public function unreadNotifications()
    {
        $unreadNotifications = auth('tourguide_api')->user()->unreadNotifications()->paginate(config('app.pagination'));
        return count($unreadNotifications) > 0
            ? NotificationResource::collection($unreadNotifications)->additional(['un_readed_notifications' => auth('tourguide_api')->user()->unreadNotifications()->count()])
            : NotificationResource::collection($unreadNotifications)->additional(['message' => __('messages.no_unread_notifications')]);
    }

    public function unreadNotificationsCount(): SuccessResource|ErrorResource
    {
        try {
            return SuccessResource::make(auth('tourguide_api')->user()->unreadNotifications()->count(), 200, 'un_readed_notifications');
        } catch (\Exception $e) {
    ...
    {
        $notification = auth('tourguide_api')->user()->notifications()->find($id);
        return $notification
    ...
    {
        try {
            if (auth('tourguide_api')->user()->unreadNotifications->count() > 0) {
                auth('tourguide_api')->user()->unreadNotifications->markAsRead();
    ...
    {
        try {
            if (auth('tourguide_api')->user()->notifications->count() > 0) {
                auth('tourguide_api')->user()->notifications()->delete();
    ...
    {
        try {
            if (auth('tourguide_api')->user()->device_key !== $request->device_key) {
                auth('tourguide_api')->user()->update(['device_key' => $request->device_key]);
    ...
    {
        return ($newsNotification = TourguideNewsNotification::find($id))
