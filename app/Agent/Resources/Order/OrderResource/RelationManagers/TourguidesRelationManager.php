...
namespace App\Agent\Resources\Order\OrderResource\RelationManagers;
        ...
        return $table
            ...
            ->headerActions([
                Tables\Actions\AttachAction::make()
                    ->label(__('actions.assign_tourguide'))
                    ->color('primary')
                    ->successNotificationTitle(__('notifications.tourguide_assigned'))
                    ...
                    ->form([
                        Forms\Components\Select::make('tourguide_id')
                            ...
                            ->placeholder(__('attributes.select', ['field' => __('attributes.tourguide')]))
                            ->rules(['required', 'exists:tourguides,id'])
                    ]),
            ])
            ->actions([
...
//                    ->visible(fn($record) => $record->status === 'approved' && $record->agent_status === 'pending')
                    ->visible(fn($record) => $record->status !== 'rejected' && $record->agent_status !== 'rejected')
